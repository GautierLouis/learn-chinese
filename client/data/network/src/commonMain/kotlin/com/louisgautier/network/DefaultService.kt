...
internal class DefaultService(
    ...
    val unauthedClient = buildClient(appConfig = appConfig, baseUrl = baseUrl, engine = engine)
    val authedClient = buildClient(appConfig = appConfig, baseUrl = baseUrl, engine = engine) {
        install(Auth) {
            bearer {
                loadTokens {
